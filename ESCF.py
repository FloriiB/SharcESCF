...
class QM(): #Q.in Class. -> read all input data and prepare turbomole calculation
    ...
    def runDefine(self): #execute Define with define.in input and fix control file in the case of GW
        ...
        # disable this in the case of slurm
        mkdir(path)
        loop_indx = 0
        while True: # wait for system to create directory and define.in later. Not really pretty...
            time.sleep(1)
            try:
                os.listdir(path)
...
def define_for_ao_ovl(path, QMin): #prepare seperate define.input to calculate AO Overlapp
    ...
    string += 'c\n all 0\n'#this works apparently, otherwise program will crash with no output
    string += '*\n'
    string += 'eht\n'
    string += 'y\n'
    string += QMin.charge + '\n'
...
def runGWBSECalculation(QMin): #run ridft, escf -gw, escf -bse and read in all the data
    ...
    if QMin.number_of_triplets != 0:
        ...
        prevdir = os.getcwd()
        triplet_dir = QMin.scratchdir +'/Triplet' # My local Turbomole version appears to have an issue if the Triplet and Singlet calculations are performed in the same directory... So move all the relevant data into a subdirectory
        mkdir(triplet_dir)
...
def runDFTCalculation(QMin): # Same as above
        ...
        if QMin.number_of_triplets != 0:
            ...
            os.popen('cp ../moments .')
            os.popen('cp ../mos .')
            time.sleep(1) # Again, not really pretty...
            ts1 = time.time()
